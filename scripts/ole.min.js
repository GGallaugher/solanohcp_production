...
OpenLayers.Editor = OpenLayers.Class({
    ...
    activeControls: [],
    editorControls: "CleanFeature DeleteFeature DeleteAllFeatures Dialog DrawHole DrawRegular DrawPolygon DrawPath DrawPoint DrawText EditorPanel ImportFeature MergeFeature SnappingSettings SplitFeature CADTools TransformFeature ContextMenu".split(" "),
    ...
    initialize: function(a, b) {
        ...
        this.editLayer.styleMap = b.styleMap ?
            b.styleMap : new OpenLayers.StyleMap({
        ...
        var c = {
            ...
            controls: ["OpenLayers.Editor.Control.DeleteFeature", "OpenLayers.Editor.Control.CleanFeature", "OpenLayers.Editor.Control.MergeFeature", "OpenLayers.Editor.Control.SplitFeature"]
        };
        ...
        for (var d = 0, e = this.featureTypes.length; d < e; d++) "polygon" == this.featureTypes[d] ? this.activeControls.push("DrawPolygon") :
            "path" == this.featureTypes[d] ? this.activeControls.push("DrawPath") : "point" == this.featureTypes[d] ? this.activeControls.push("DrawPoint") : "regular" == this.featureTypes[d] ? this.activeControls.push("DrawRegular") : "text" == this.featureTypes[d] && this.activeControls.push("DrawText");
        d = 0;
        for (e = this.sourceLayers.length; d < e; d++) c = {
            ...
            c, this.selectionChanged), this.sourceLayers[d].styleMap = new OpenLayers.StyleMap({
            "default": new OpenLayers.Style({
                fillColor: "#0c0",
                fillOpacity: 0.8,
                strokeColor: "#070",
    ...
    selectionChanged: function() {
        ...
        if (0 < this.layer.selectedFeatures.length && a && a.active)
            for (var a = 0, b = this.controls.length; a < b; a++) {
                ...
                c && OpenLayers.Element.removeClass(c.panel_div,
                    "oleControlDisabled")
            } else {
                a = 0;
                for (b = this.controls.length; a < b; a++)(c = this.editor.editorPanel.getControlsByClass(this.controls[a])[0]) && OpenLayers.Element.addClass(c.panel_div, "oleControlDisabled")
            }
    ...
    addEditorControls: function() {
        for (var a = null, b = [], c = 0, d = this.activeControls.length; c < d; c++) {
            a = this.activeControls[c]; - 1 < OpenLayers.Util.indexOf(this.editorControls,
                a) && b.push(new OpenLayers.Editor.Control[a](this.editLayer, this.options[a]));
            switch (a) {
                case "Separator":
                ...
                case "DragFeature":
                    ...
                    break;
                case "ModifyFeature":
                    b.push(new OpenLayers.Control.ModifyFeature(this.editLayer,
                        OpenLayers.Util.extend({
                            title: OpenLayers.i18n("oleModifyFeature")
                        }, this.options.ModifyFeature)));
                    break;
                case "SelectFeature":
                    b.push(new OpenLayers.Control.SelectFeature(this.sourceLayers.concat([this.editLayer]), OpenLayers.Util.extend({
                ...
                case "DownloadFeature":
                    b.push(new OpenLayers.Editor.Control.DownloadFeature(this.editLayer, OpenLayers.Util.extend({},
                ...
                case "UploadFeature":
                    ...
                    break;
                default:
                    -1 == OpenLayers.Util.indexOf(this.editorControls, a) && OpenLayers.Editor.Control[a] && b.push(new OpenLayers.Editor.Control[a](this.editLayer, OpenLayers.Util.extend({}, this.options[a])))
            }
    ...
    createEditorPanel: function(a) {
        if (this.controls.ContextMenu) {
            for (var b = this.controls.ContextMenu.contextMenuControls || [], c = b.length; c--;) {
                var d = a.indexOf(this.controls[b[c]]);
                ~d && a.splice(d, 1)
            }
            a.splice(a.indexOf(this.controls.ContextMenu), 1)
        }
        b = new OpenLayers.Editor.Control.EditorPanel(this);
        b.addControls(a);
        return b
    },
    status: function(a) {
        "error" == a.type && alert(a.content)
    },
...
OpenLayers.Editor.Control.DeleteFeature = OpenLayers.Class(OpenLayers.Control.Button, {
    ...
    deleteFeature: function() {
        if (0 < this.layer.selectedFeatures.length) {
            for (var a = 0; a < this.layer.selectedFeatures.length; a++) {
                var b =
                    this.layer.selectedFeatures[a];
...
OpenLayers.Editor.Control.DrawRegular = OpenLayers.Class(OpenLayers.Control.DrawFeature, {
    ...
    activate: function() {
        ...
        e.htmlFor = "oleCADToolsDialogIrregular";
        e.appendChild(document.createTextNode(OpenLayers.i18n("oleDrawRegularIrregular")));
        c.appendChild(e);
        b.appendChild(c);
        c = document.createElement("div");
        var f = document.createElement("select");
        f.id = "oleCADToolsDialogSides";
        for (e = 0; e < this.sides.length; ++e) {
            var g = 20 > this.sides[e] ? OpenLayers.i18n("oleDrawRegularSides" + this.sides[e]) : OpenLayers.i18n("oleDrawRegularCircle");
            f.options.add(new Option(g, this.sides[e]))
        }
        OpenLayers.Event.observe(f, "change", OpenLayers.Function.bind(function(a) {
                this.handler.setOptions({
                    sides: parseInt(f.options[f.selectedIndex].value)
                })
            },
            this));
        this.handler.setOptions({
            sides: parseInt(f.options[0].value)
        });
        c.appendChild(f);
        e = document.createElement("label");
        e.htmlFor = "oleCADToolsDialogSides";
        e.appendChild(document.createTextNode(OpenLayers.i18n("oleDrawRegularShape")));
        c.appendChild(e);
        b.appendChild(c);
...
OpenLayers.Editor.Control.ContextMenu = OpenLayers.Class(OpenLayers.Control.Panel, {
    ...
    contextMenuControls: "CleanFeature DeleteFeature DrawHole MergeFeature SplitFeature TransformFeature".split(" "),
    initialize: function(a, b) {
        a.events.register("featureselected", this, this.openContextMenu);
        a.events.register("featureunselected", this, this.closeContextMenu);
        a.map.events.register("move", this, this.updatePosition);
        OpenLayers.Control.Panel.prototype.initialize.apply(this, [b]);
        a.map.addControl(this)
    },
    addContextMenuControls: function() {
        for (var a = [], b = 0; b < this.contextMenuControls.length; b++) {
            var c = this.map.editor.controls[this.contextMenuControls[b]];
            c && a.push(c)
        }
        this.addControls(a)
    },
    openContextMenu: function() {
        ...
        this.activate();
        this.updatePosition();
        for (var a = 0; a < this.controls.length; a++) OpenLayers.Element.removeClass(this.controls[a].panel_div, "oleControlDisabled")
    },
    closeContextMenu: function() {
        this.map.editor.editLayer.selectedFeatures.length ||
            (this.div.style.display = "none", this.deactivate())
    },
    updatePosition: function() {
        var a = this.map.editor.editLayer.selectedFeatures;
        if (a.length) {
            for (var b = new OpenLayers.Bounds, c = this.map.getExtent().getCenterLonLat(), c = new OpenLayers.Geometry.Point(c.lon, c.lat), d = 0; d < (a || []).length; d++) b.extend(a[d].geometry.getBounds());
            a = b.toGeometry().distanceTo(c, {
                details: !0
            }) || {
                x0: b.left,
                y0: b.top
            };
            c = this.map.getPixelFromLonLat(new OpenLayers.LonLat(a.x0, a.y0));
            this.div.style.right = "initial";
            this.div.style.left = c.x +
                "px";
            this.div.style.top = c.y + "px";
            a.y0 === b.top ? this.div.style.top = parseInt(this.div.style.top) - this.menuBufferDistance - this.div.clientHeight + "px" : a.y0 === b.bottom && (this.div.style.top = parseInt(this.div.style.top) + this.menuBufferDistance + "px");
            a.x0 === b.left ? this.div.style.left = parseInt(this.div.style.left) - this.menuBufferDistance - this.div.clientWidth + "px" : a.x0 === b.right && (this.div.style.left = parseInt(this.div.style.left) + this.menuBufferDistance + "px")
        }
    },
